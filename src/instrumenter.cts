...
export default async function run(url: string, options: Options) {
  ...
  if (options.customInstrumentation === true) {
    ...
    fss.writeFileSync(path.join(binPath, binName), new Int8Array(results[0].wasm))
    ...
    execSync(`./target/debug/replay_gen ${tracePath} ${path.join(binPath, binName)} true`);
