...
export default async function run(url: string, options: Options) {
  ...
  if (options.file !== undefined) {
    const code = await new Generator().generateReplayFromStream(
      fss.createReadStream(options.file)
    );
  ...
  if (options.customInstrumentation === true) {
    ...
    analyser = new CustomAnalyser(options.benchmarkPath, {
      javascript: true,
    ...
    analyser = new Analyser("./dist/src/tracer.cjs", {
    ...
    const results = await analyser.stop();
    console.log("Download done. Generating Benchmark...");
    Benchmark.fromAnalysisResult(results).save(options.benchmarkPath, {
      trace: options.dumpTrace,
