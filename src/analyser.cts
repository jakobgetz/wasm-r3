...
export class Analyser implements AnalyserI {
  ...
  constructor(
    analysisPath: string,
    options: Options = { extended: false, noRecord: false }
  ) {
  ...
  async start(url, { headless } = { headless: false }) {
    ...
    this.browser = await chromium.launch({
      ...
      headless,
      args: ["--experimental-wasm-multi-memory"],
    });
...
export class CustomAnalyser implements AnalyserI {
  ...
  private disableShadowOpt: boolean;

  constructor(
    benchmarkPath: string,
    options: { javascript: boolean; disableShadowOpt: boolean }
  ) {
    this.benchmarkPath = benchmarkPath;
    this.javascript = options.javascript;
    this.disableShadowOpt = options.disableShadowOpt;
  ...
  async start(url: string, { headless } = { headless: false }) {
    ...
    this.p_measureUserInteraction = createMeasure("user interaction", {
      phase: "record",
      description: `The time the user interacts with the webpage during recording, from the time the 'load' event got fired in the browser until the user stops the recording.`,
    });
    return this.page;
  }
  ...
  async stop(): Promise<AnalysisResult> {
    ...
    originalWasmBuffer.forEach(({ href, buffer }, i) => {
      ...
      fss.writeFileSync(
        path.join(subBenchmarkPath, binName),
      ...
      execSync(
        `./target/release/replay_gen stringify ${tracePath} ${binPath} ${traceTextPath}`
      );
      console.log(i, "done");
      console.log(i, "generate replay...");
      if (this.javascript) {
        execSync(
          `./target/release/replay_gen generate ${tracePath} ${binPath} true ${jsPath}`
        );
      } else {
        execSync(
          `./target/release/replay_gen generate ${tracePath} ${binPath} true ${wasmPath}`
        );
      }
  ...
  private async getBuffers() {
    ...
    return originalWasmBuffer.flat(1).filter((v) => v != undefined) as {
      buffer: any;
      href: string;
    }[];
  }
  ...
  private async constructInitScript() {
    ...
    let script = tracerScript + ";\n";
    if (this.disableShadowOpt === true) {
      script += "var disableShadowOpt = true;\n";
    } else {
      script += "var disableShadowOpt = false;\n";
    }
    return script + setupScript + ";\n";
  }
}
