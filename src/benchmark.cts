      ...
      this.record.map(async ({ binary, trace, statistic }, i) => {
        ...
        if (options.legacyBackend == true) {
          ...
          if (options.jsBackend) {
            execSync(
              `./crates/target/debug/replay_gen ${diskSave} ${path.join(
                binPath,
                "index.wasm"
              )} ${path.join(binPath, "replay.js")}`
            );
          } else {
            execSync(
              `./crates/target/debug/replay_gen ${diskSave} ${path.join(
                binPath,
                "index.wasm"
              )} ${path.join(binPath, "replay.wasm")}`
            );
            execSync(`node ${path.join(binPath, "replay.js")}`, {
              cwd: binPath,
            });
            execSync(
              `wasm-tools validate -f all ${path.join(binPath, "replay.wasm")}`
            );
