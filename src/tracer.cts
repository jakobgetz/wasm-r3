...
export class Trace {
    ...
    constructor() {
        ...
        class Analysis {
            ...
            mem_content_equals(memIdx, addr, numBytes, data) {
                let result = [];
        ...
        //# sourceMappingURL=tracer-worker.cjs.map`;
...
export default class Analysis implements AnalysisI<Trace> {
    ...
    constructor(Wasabi: Wasabi, options = { extended: false }) {
        ...
        Wasabi.analysis = {
            ...
            begin_function: (location, args) => {
                this.trace.push(`FE;${location.func};${args.join(',')}`)
                // this.trace.push({ type: 'begin_function', func: location.func, args })
            },

            store: (location, op, target, memarg, value) => {
                this.trace.push({ type: 'store', op, target, memarg })
            },

            memory_grow: (location, memIdx, byPages, previousSizePages) => {
                this.trace.push({ type: 'memory_grow', memIdx, byPages })
            },

            load: (location, op, target, memarg, value) => {
                this.trace.push(`LE;${0};${memName};${addr};${data.join(',')}`)
                // this.trace.push({ type: 'load', op, target, memarg })

            },

            global: (location, op, idx, value) => {
                this.trace.push(`G;${idx};${this.getName(globalInfo)};${value};${valtype}`)
                // this.trace.push({ type: 'global', op, idx, value })
            },

            call_pre: (location, op, funcidx, args, tableTarget) => {
                this.trace.push({ type: 'call_pre', op, funcidx, tableTarget })
            },

            call_post: (location, results) => {
                this.trace.push({ type: 'call_post', results })
            },

            return_: (location, values) => {
                this.trace.push(`FR;${location.func};${values.join(',')}`)
                // this.trace.push({ type: 'return', location, values })
            },

            table_set: (location, target, value) => {
                this.trace.push({ type: 'table_set', target, value })
            },

            table_get: (location, target, value) => {
                this.trace.push({ type: 'table_get', target })
            }
        }
    }

    init() {
        this.trace.push({ type: 'info', info: this.Wasabi.module.info })
        // Init Memories
        ...
        // this.trace.push({ type: 'memories', memories: this.shadowMemories })
        // this.trace.push({ type: 'tables', tables: this.shadowTables })
        // this.trace.push({ type: 'globals', globals: this.shadowGlobals })
        // Init Functions
